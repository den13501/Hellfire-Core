...
void Map::ApplyBuffs(Player* player, Map* map, float difficulty, int dunLevel, int numInGroup)
{
    ...
    if (difficulty != 0)
    {
        ...
        if (player->GetLevel() <= dunLevel + SolocraftLevelDiff) //If a player is too high level for dungeon don't buff but if in a group will count towards the group offset balancing.
        {
            ...
            QueryResultAutoPtr result = RealmDataDatabase.PQuery("SELECT `GUID`, `Difficulty`, `GroupSize`, `SpellPower`, `Stats` FROM `custom_solocraft_character_stats` WHERE GUID = %u", player->GetGUIDLow());
            ...
            RealmDataDatabase.PExecute("REPLACE INTO custom_solocraft_character_stats (GUID, Difficulty, GroupSize, SpellPower, Stats) VALUES (%u, %f, %u, %i, %f)", player->GetGUIDLow(), difficulty, numInGroup, SpellPowerBonus, SoloCraftStatsMult);
        }
        else
        {
    ...
    if (group)
    {
        ...
        for (Group::member_citerator itr = groupMembers.begin(); itr != groupMembers.end(); ++itr)
        {
            ...
            if (itr->guid != player->GetGUIDLow())
            {
                ...
                QueryResultAutoPtr result = RealmDataDatabase.PQuery("SELECT `GUID`, `Difficulty`, `GroupSize` FROM `custom_solocraft_character_stats` WHERE GUID = %u", itr->guid);
                if (result)
                {
...
void Map::ClearBuffs(Player* player, Map* map)
{
    ...
    if (result)
    {
        ...
        RealmDataDatabase.PExecute("DELETE FROM custom_solocraft_character_stats WHERE GUID = %u", player->GetGUIDLow());
    }
}