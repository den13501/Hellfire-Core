...
void WorldSession::HandleCharCreateOpcode(WorldPacket & recv_data)
{
    ...
    if (!HasHigherGMLevel(SEC_GAMEMASTER))
    {
    ...
    if (!HasHigherGMLevel(SEC_GAMEMASTER) && sObjectMgr.IsReservedName(name,GetAccountId()))
    {
    ...
    bool AllowTwoSideAccounts = !sWorld.IsPvPRealm() || sWorld.getConfig(CONFIG_ALLOW_TWO_SIDE_ACCOUNTS) || HasHigherGMLevel(SEC_DEVELOPER);
...
void WorldSession::HandleChangePlayerNameOpcode(WorldPacket& recv_data)
{
    ...
    // check name limitations
    if (!HasHigherGMLevel(SEC_GAMEMASTER) && sObjectMgr.IsReservedName(newname, GetAccountId()))
    {
        WorldPacket data(SMSG_CHAR_RENAME, 1);
        data << uint8(CHAR_NAME_RESERVED);
        SendPacket(&data);
